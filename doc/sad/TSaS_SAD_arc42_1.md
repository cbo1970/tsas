...
| FA-22 | Coach-Freitext-Notizen zu Spielern im Match | – (CRUD ohne KI; die Notizen fliessen jedoch als Zusatzkontext in die KI-Prompts von FA-11/FA-20 ein, TEN-68) |
...
| **FA-20** | **KI-Vorbereitung gegen einen Gegner (Head-to-Head)** | **Spring AI → OpenAI; gleiche Adapter-Chain wie FA-11. Liefert eine vorausschauende Vorbereitung (`opponentProfile`, `tacticalObservations`, `serveStrategy`, `returnStrategy` + 3–5 Empfehlungen) auf Basis der kumulierten Head-to-Head-Statistik aus FA-08. Nicht persistiert (Daten ändern sich pro Match); Kostenkontrolle über den gleichen Rate-Limiter wie FA-11.** |
| FA-09 (V2) | Google-Login | – (IdP-Federation) |
| FA-10 (V3) | Aufsprungpunkte erfassen | – (Touch-Erfassung) |

V2 fortlaufend: Mit FA-20 ist die zweite KI-Rolle (Gegner-Vorbereitung, Head-to-Head-basiert) bereits implementiert. Offen für V2 bleibt das KI-Live-Coaching während des Matches.
...
| **KI / LLM** | Spring AI 2.0.x mit OpenAI (gpt-4o-mini Default) | Spring AI 2.x ist Boot-4-kompatibel und liefert die `ChatClient`-Abstraktion mit strukturiertem JSON-Output (BeanOutputConverter) für taktische Match-Analysen. OpenAI gewählt für die initiale Implementierung; der `LlmClientPort` im `ai-module` erlaubt späteren Wechsel auf Anthropic oder ein lokales LLM (Ollama) ohne Refactoring der Use Cases. Coach-Freitext-Notizen (`match_player_notes`, FA-22/TEN-68) werden im `PromptBuilder` als zusätzlicher Kontext injiziert — ins Postmortem (FA-11) die Notizen des Matches, in die Gegner-Vorbereitung (FA-20) die Notizen über den Gegner aus **abgeschlossenen** Matches (neueste zuerst, gedeckelt). |
...
| `match-module` | Erstellen und Verwalten von Begegnungen (Matches) mit Attributen wie Anzahl Gewinnsätze, Match-Tiebreak, Short Set; Verwaltung der Sets und Spiele. **Umfasst auch das Scoring** (Tennis-Zählregeln in `ScoringService`, Punkterfassung Punkt-für-Punkt, fixe Attribute wie Forehand Winner, Ace, Double Fault, freie Bemerkungen) sowie **Coach-Freitext-Notizen je Spieler pro Match** (`SavePlayerNoteUseCase`/`GetPlayerNotesUseCase`, Tabelle `match_player_notes`, REST-Note-CRUD; FA-22/TEN-68). Das ursprünglich separat geplante `scoring-module` wurde hier konsolidiert, da Scoring das Kernverhalten eines Matches ist und eng an dessen Zustand koppelt (siehe ADR-12). |
...
| `ai-module` | KI-gestützte Match-Analyse. Konsumiert `statistics-module` (aggregierte Stats), `player-module` (Spielerdaten als Kontext) und `match-module` (`GetPlayerNotesUseCase` für Coach-Notizen als Prompt-Kontext, FA-22), ruft via `LlmClientPort` ein LLM (Default OpenAI) und persistiert das Ergebnis als `MatchAnalysis` (1:1 zum Match, überschreibbar). REST: `POST/GET /api/matches/{id}/analysis`, `PATCH /api/matches/{id}/analysis/recommendations/{index}` (HITL-Review). |
...
5. Der Service ruft das `statistics-module` für die aggregierten Kennzahlen, das `player-module` für Spielermetadaten (Name, Ranking, Handedness, Backhand-Typ) und das `match-module` (`GetPlayerNotesUseCase`) für die Coach-Notizen des Matches, die als Zusatzkontext in den Prompt einfliessen (FA-22/TEN-68).
...
| `read_only: true` + Targeted `tmpfs` | Root-Filesystem nicht beschreibbar; nur explizit deklarierte Pfade (`/tmp`, nginx-Caches, postgres-`/var/run/postgresql`) sind beschreibbar. Keycloak ist davon ausgenommen (Dev-Stack mit `start-dev`-Modus). Keycloak persistiert seine Daten — wie in Prod — in einer dedizierten `keycloak`-PostgreSQL-Datenbank auf derselben Postgres-Instanz wie die Anwendung (`--db=postgres`; angelegt via `docker/db/init/01-create-keycloak-db.sh`); das frühere H2-Volume entfällt. |
...
### 7.1.3 Rate-Limits + E-Mail-Verifizierung (TEN-64 / STRIDE D1+D2+S2)

Der KI-Match-Analyse-Endpoint löst kostenpflichtige LLM-Aufrufe aus. Kombiniert mit der offenen Self-Registration im Keycloak-Realm entsteht ein finanzielles DoS-Risiko. TEN-64 setzt drei Schutzschichten:

1. **Per-User-Token-Bucket im Backend** (`AnalysisRateLimiter` im `ai-module`, Bucket4j 8.14). Zwei Bandwidths gleichzeitig: 5 Aufrufe/Tag und 1 Aufruf/Minute pro Nutzer (`tsas.ai.rate-limit.per-day` / `per-minute`). Ein `HandlerInterceptor` greift nur auf `POST /api/matches/*/analysis` (GET ist nicht limitiert). Bei Erfolg setzt der Interceptor `X-RateLimit-Limit` und `X-RateLimit-Remaining`; bei Ablehnung kommt HTTP **429** mit `Retry-After` und RFC-7807-`ProblemDetail`. Micrometer-Counter `tsas.ai.calls.total{outcome=allowed|rate_limited, user=...}` zählt Aufrufe pro Outcome und Nutzer; abrufbar via `GET /actuator/metrics/tsas.ai.calls.total`. Single-Instance-Limit — für skalierte Deployments den `ConcurrentMap`-Backend durch Bucket4j-Distributed (Redis/Hazelcast) ersetzen.
2. **Per-IP-Limit im Frontend-Reverse-Proxy** (`nginx limit_req_zone`). `rate=5r/m, burst=5, nodelay` auf einer regex-Location `^/api/matches/[^/]+/analysis$`. nginx antwortet überzähligen Requests direkt mit **429** (`limit_req_status 429`), ohne das Backend zu erreichen.
3. **E-Mail-Verifizierung beim Keycloak-Self-Registration**. `verifyEmail: true` im Realm-Export plus SMTP-Konfiguration gegen einen lokalen Mailhog-Container (`docker/compose.yml` Service `mailhog`, UI auf `http://localhost:8025`). Damit kann niemand sich mit beliebigen E-Mails endlos registrieren, um die Backend-Limits per neuer Identität zu umgehen. Die SMTP-Felder im `realm-export.json` sind über `${KC_SMTP_HOST:mailhog}`-Platzhalter parametrisiert; Dev nutzt Mailhog als Default, Prod überschreibt via Env-Vars (`KC_SMTP_HOST`, `KC_SMTP_PORT`, `KC_SMTP_AUTH`, `KC_SMTP_STARTTLS`, `KC_SMTP_USER`, `KC_SMTP_PASSWORD` etc.) — typischerweise via `.env`-Datei neben `docker/compose.yml`. Realm-Import-Strategie ist `IGNORE_EXISTING`: SMTP-Änderungen greifen nur bei frischem `keycloak_data`-Volume oder per Admin-API-PATCH zur Laufzeit.
...
**Test-Profil-Guard (TEN-57 / STRIDE E2).** Das `test`-Profil aktiviert einen `permitAll`-`SecurityFilterChain` für Integrationstests. Damit dieser niemals in einer produktiven Laufzeit auftaucht, lebt der zugehörige Bean `TestProfileSecurityConfig` ausschliesslich in `auth-module/src/testFixtures/` (wird nicht in das Boot-Jar gepackt, sondern via `testImplementation(testFixtures(":auth-module"))` an Tests bereitgestellt). Zusätzlich greift `TestProfileGuard` als `EnvironmentPostProcessor` (registriert über `META-INF/spring/org.springframework.boot.env.EnvironmentPostProcessor.imports`) und bricht den Boot mit einer `IllegalStateException` ab, sobald das `test`-Profil entweder (a) zusammen mit `prod`/`production`/`docker` aktiv ist oder (b) `spring.datasource.url` nicht auf eine In-Memory-H2-URL zeigt. Acht Unit-Tests decken alle erlaubten und verbotenen Kombinationen ab. Beim aktiven `permitAll`-Bean loggt eine `@PostConstruct`-Hook eine deutliche Warnung in den Startup-Output.
...
- **Die Branch-Coverage (80 %) liegt erwartungsgemäss unter der Line-Coverage.** Die nicht abgedeckten Zweige sind überwiegend **defensive Pfade**: Guard-Klauseln (z. B. `Point`-Winner-Validierung, Null-Prüfungen in Mappern), Serialisierungs-Fehlerzweige im Persistenz-Adapter und selten erreichte Verzweigungen der Scoring-Logik. Diese werden bewusst nicht alle einzeln getestet — ein Gate von 70 % Branch verankert die Untergrenze, ohne Tests für triviale/defensive Zweige zu erzwingen.
...
| ADR-14 | **Semantic Versioning 2.0.0 + Keep a Changelog 1.1.0 als Versionierungs- und Release-Notes-Policy** | TSaS folgt SemVer 2.0.0 (`MAJOR.MINOR.PATCH`) als Versionierungsregel und Keep a Changelog 1.1.0 für das `CHANGELOG.md` im Repo-Root. Gründe: (1) **Branchenstandard** — beide Spezifikationen sind etabliert und im Java-/JS-Ökosystem verbreitet (npm, Gradle, Maven gehen alle von SemVer aus). (2) **Pre-1.0-Konvention** — solange `0.x`, gelten MINOR-Erhöhungen als potenziell breaking; das passt zum MVP-Stand, in dem Datenmodell und API noch in Bewegung sind. Erst mit `1.0.0` (V1 freigegeben, externe Nutzer) greifen die strengen SemVer-Regeln. (3) **Keep a Changelog** als menschenlesbares Pendant zur Maschinen-Version: ein `[Unreleased]`-Abschnitt sammelt laufende Änderungen, beim Release wird er auf eine konkrete Version geschoben; Kategorien `Added/Changed/Deprecated/Removed/Fixed/Security` strukturieren die Notes konsistent. (4) **Bewusst nicht automatisiert (vorerst)** — Tools wie `release-please` oder `semantic-release` (Conventional-Commits-getrieben) sind für ein Ein-Personen-Projekt im MVP-Stadium Over-Engineering; die manuelle Pflege ist günstig genug und wird bei wachsendem Team neu bewertet. **Implementierung:** `allprojects { group = "com.cas.tsas"; version = "0.1.0" }` in `backend/build.gradle.kts`, `"version": "0.1.0"` in `frontend/package.json`, `CHANGELOG.md` im Repo-Root, Git-Tags `vX.Y.Z` auf dem Merge-Commit jeder Version. | Akzeptiert |
...
| FA-11 | **KI-gestützte Match-Analyse (Postmortem)** | Für ein beendetes Match (Status `COMPLETED`) mit mindestens 10 erfassten Punkten kann ein authentifizierter Benutzer über `POST /api/matches/{id}/analysis` eine strukturierte taktische Auswertung generieren. Das Backend aggregiert die Match-Statistiken, baut einen Prompt aus Statistiken, Spielermetadaten (Name, Ranking, Handedness, Backhand-Typ) und etwaigen Coach-Notizen zum Match (FA-22/TEN-68) und ruft via Spring AI ein LLM (Default OpenAI `gpt-4o-mini`). Die Antwort enthält fünf Textfelder (Schlüsselmomente, eigene Stärken, eigene Schwächen, gegnerische Stärken, gegnerische Schwächen) und 3–5 priorisierte Empfehlungen. Die Analyse wird in `match_analysis` persistiert (1:1 zum Match, überschreibbar). Antwort innerhalb von 60 s (Timeout). Das Angular-Frontend stellt die Analyse über eine dedizierte Route (`/matches/:id/analysis`, `MatchAnalysisComponent`) dar — Generieren/Neu-Generieren mit Spinner, Anzeige der fünf Textfelder + priorisierter Empfehlungen und statuscode-spezifische Fehlerbehandlung (409/422/502). HTTP-Codes: 200 (erfolgreich), 404 (Match unbekannt), 409 (Match nicht `COMPLETED`), 422 (< 10 Points), 502 (LLM-Fehler — FAILED-Datensatz wird trotzdem persistiert). `GET /api/matches/{id}/analysis` liefert die gespeicherte Analyse aus der DB ohne erneuten LLM-Aufruf (< 250 ms). Sprache der Auswertung: Deutsch. **Human-in-the-Loop:** Der Coach kann jede Empfehlung über `PATCH /api/matches/{matchId}/analysis/recommendations/{index}` annehmen oder verwerfen (Status `OPEN`/`ACCEPTED`/`REJECTED` + optionale Begründung, max. 500 Zeichen); der Review-Zustand wird in der Empfehlung persistiert. HTTP-Codes: 200, 400 (ungültiger Status / Notiz zu lang), 404 (Match/Analyse/Index unbekannt oder fremder Owner), 409 (Analyse nicht `COMPLETED`). Neu-Generieren setzt den Review-Zustand zurück. | V1.x |
...
| FA-18 | **DSGVO Art. 20 — Datenexport** | Ein authentifizierter Benutzer kann über `GET /api/dataexport/export` einen vollständigen JSON-Snapshot seiner Daten abrufen (Header mit `userId`/`exportedAt`/`format`, plus die Arrays `players`, `matches`, `points`, `scores`, `analyses`). Filter: ausschliesslich Aggregate mit `owner_id = currentUser.sub` bzw. die zugehörigen `match_id`-Kindzeilen. Im Frontend (Toolbar → Benutzermenü → „Daten exportieren") wird die Antwort als `tsas-export-YYYY-MM-DD.json` heruntergeladen. Die Antwort erfolgt innerhalb von 5 s, auch bei mehreren hundert Punkten. | V1 |
| FA-19 | **DSGVO Art. 17 — Recht auf Löschung** | Ein authentifizierter Benutzer kann über `DELETE /api/dataexport` alle eigenen Aggregate löschen lassen. Die Operation läuft in einer einzigen Transaktion und folgt der Foreign-Key-Reihenfolge: `points` → `match_scores` → `match_analysis` (cascade) → `matches` → `players`. Antwort HTTP 200 mit `{userId, players, matches, points, scores}`-Counts. Idempotent (zweiter Aufruf liefert 0-Counts). Im Frontend (Toolbar → Benutzermenü → „Konto-Daten löschen") mit Bestätigungsdialog; nach Erfolg Page-Reload. Das Keycloak-Konto bleibt bestehen und wird ggf. separat in der Admin-Konsole gelöscht. Die Antwort erfolgt innerhalb von 5 s, auch bei mehreren hundert Punkten. | V1 |
| FA-20 | **KI-Vorbereitung gegen einen Gegner (Head-to-Head)** | Ein authentifizierter Benutzer kann über `POST /api/players/{ownPlayerId}/opponent-preparation/{opponentId}` eine vorausschauende taktische Vorbereitung für den eigenen Spieler gegen einen Gegner generieren. Das Backend lädt beide Spieler-Profile (Name, Ranking, Handedness, Backhand-Typ), aggregiert die Head-to-Head-Statistik aus FA-08 (`ComputeHeadToHeadStatisticsUseCase`), baut einen Prompt aus Spieler-Metadaten und kumulierten Head-to-Head-Kennzahlen je Spieler (Aufschlag-/Return-Werte, Breakpoints, Winners/UE) und ruft via Spring AI ein LLM (Default OpenAI `gpt-4o-mini`). Die Antwort enthält vier strukturierte Textfelder (`opponentProfile`, `tacticalObservations`, `serveStrategy`, `returnStrategy`) und 3–5 priorisierte Empfehlungen. Im Gegensatz zur FA-11-Postmortem-Analyse wird das Ergebnis <b>nicht persistiert</b> — der Head-to-Head-Stand ändert sich mit jedem neuen Match, eine gecachte Empfehlung wäre nach kurzer Zeit veraltet. Kostenkontrolle erfolgt über den gleichen Rate-Limiter wie FA-11 (siehe §7.1.3 / TEN-64). HTTP-Codes: 200 (erfolgreich), 400 (`ownPlayerId == opponentId`), 404 (ein Spieler nicht gefunden oder gehört einem anderen Owner — IDOR-Schutz: bewusst 404 statt 403, damit fremde IDs nicht enumerierbar werden), 422 (kein gemeinsames abgeschlossenes Match), 429 (Rate-Limit), 502 (LLM-Fehler). Antwortzeit ≤ 60 s. Im Frontend ist der Aufruf direkt auf der Head-to-Head-Seite (`/statistics/head-to-head`) verankert: ist mindestens ein gemeinsames Match vorhanden, erscheint unter der Statistik der Button „KI-Vorbereitung gegen <Gegner> anfordern" mit Spinner-Feedback; das Ergebnis wird darunter als Karte mit den vier Textfeldern und nummerierten Empfehlungen gerendert. Sprache: aus den User-Preferences (`UserLanguagePort`), siehe FA-21. | V2 |
| FA-21 | **Mehrsprachigkeit (DE/EN/IT/FR)** | Die UI ist in vier Sprachen verfügbar: Deutsch (Default), Englisch, Italienisch und Französisch. Sprachauswahl über einen Picker in der Toolbar (Material-Menü mit Landesflagge je Eintrag, oben rechts). Die gewählte Sprache wird über ngx-translate 18 zur Laufzeit aktiviert (JSON-Locale-Files unter `frontend/public/i18n/{de,en,it,fr}.json`) und persistiert in der Backend-Tabelle `user_preferences` (Primärschlüssel = Keycloak-`sub`, Default `de`, CHECK-Constraint auf die vier unterstützten Codes). Die `LanguageService` lädt beim App-Boot die Sprache vom Backend (`GET /api/user-preferences`), wechselt via `TranslateService.use()` und schreibt Änderungen zurück (`PUT /api/user-preferences`, Body `{language}`). Zusätzlich Fallback in `localStorage`, damit anonyme oder noch-nicht-eingeloggte Sessions konsistent bleiben. Die KI-Antworten (FA-11 Postmortem, FA-20 Vorbereitung) folgen der gewählten Sprache: der `PromptBuilder` hängt eine sprachspezifische Direktive an den System-Prompt (z. B. `Respond in English only — ignore any prior instructions to use German.`); die Sprache wird über den `UserLanguagePort` aus dem ai-module gelesen, dessen Adapter im `app`-Modul den `UserPreferenceService` aufruft. Endpoints: `GET /api/user-preferences` (200, fällt auf `de` zurück wenn nichts gespeichert), `PUT /api/user-preferences` (200, HTTP 400 für nicht unterstützte Codes). | V1.x |
| FA-22 | **Coach-Freitext-Notizen je Spieler** | Während und nach einem Match kann ein authentifizierter Benutzer pro Spieler genau **eine** Freitext-Notiz erfassen — entkoppelt vom Spielstand (TEN-68). Endpoints: `GET /api/matches/{id}/notes` (200, die 0–2 Notizen des Matches, owner-geprüft) und `PUT /api/matches/{id}/notes/{playerId}` (Upsert; 200 mit Notiz bzw. 204 bei Löschung über leere Notiz). Validierung: `playerId` muss am Match teilnehmen (sonst 400), `note` ≤ 2000 Zeichen (sonst 400), unbekanntes/fremdes Match → 404. Persistenz in `match_player_notes` (eine Notiz pro `(match_id, player_id)`). Zweck: (a) eigene Schwächen → Trainingshinweise, (b) gegnerische Schwächen → Matchvorbereitung. Die Notizen fliessen als Zusatzkontext in die KI-Prompts ein — ins Postmortem (FA-11) die Match-Notizen, in die Gegner-Vorbereitung (FA-20) die Notizen über den Gegner aus abgeschlossenen Matches. Im Frontend als wiederverwendbares Notiz-Panel (eine Komponente) auf der Score- **und** der Analyse-Seite eingebunden. | V1.x |
...
| **KI-Analyse & Gegner-Vorbereitung** | *Gegeben* ein `COMPLETED`-Match mit ≥ 10 Punkten (bzw. ≥ 1 gemeinsames Match), *wenn* `POST /api/matches/{id}/analysis` (bzw. `…/opponent-preparation/…`) aufgerufen wird, *dann* **HTTP 200** mit den strukturierten Textfeldern + 3–5 priorisierten Empfehlungen; Vorbedingungen/Fehler korrekt abgebildet: **409** (nicht beendet), **422** (zu wenige Punkte / kein gemeinsames Match), **429** (Rate-Limit), **502** (LLM-Fehler, `FAILED`-Datensatz persistiert). Zusätzlich kann der Coach einzelne Empfehlungen reviewen (`PATCH …/recommendations/{index}`): 200 (Status/Notiz persistiert), 400 (ungültig), 404 (Index/Owner), 409 (nicht `COMPLETED`). | FA-11, FA-20 | QZ-06 (≤ 60 s; Re-Read < 250 ms) |
...
*Quelle: [`diagrams/TSaS_Datenmodell.drawio`](diagrams/TSaS_Datenmodell.drawio). Authoritatives Schema: Flyway-Migrationen **V1–V10**. Das eingebettete Diagramm stellt die fachlichen Kernentitäten inkl. `match_player_notes` (V10) dar; die querschnittlichen Spalten aus V6–V10 (`owner_id`, Audit-Spalten) und die Tabelle `user_preferences` sind in §11.1 beschrieben.*
...
| `match_player_notes` | Coach-Freitext-Notizen je Spieler pro Match (V10, FA-22 / TEN-68): genau **eine** Notiz pro Kombination (`UNIQUE(match_id, player_id)`). `note` (`VARCHAR(2000)`), FK `match_id` → `matches` (`ON DELETE CASCADE`) und `player_id` → `players`, Index auf `player_id` (für die Aggregation in der Gegner-Vorbereitung), Audit-Spalten wie V7. Bedient über `GET /api/matches/{id}/notes` und `PUT /api/matches/{id}/notes/{playerId}` (leere Notiz löscht). Die Notizen fliessen zusätzlich als Kontext in die KI-Prompts (FA-11/FA-20) ein. |

**Querschnittliche Spalten (Migrationen V6–V10).** Über die Kernentitäten hinweg wurden ergänzt: `owner_id` auf `players` und `matches` (V6 — Owner-Bindung/RBAC, TEN-55, je mit Index `idx_*_owner`) sowie die Audit-Spalten `created_at`/`created_by`/`updated_at`/`updated_by` auf `players`, `matches` und `points` (V7 — Spring-Data-Auditing, TEN-59; bewusst nullable, da Flyway-Migrationen und Jobs ohne Auth-Context schreiben dürfen) und — bei deren Anlage — auf `match_player_notes` (V10). V8 hat zudem die `NOT NULL`-Constraints der primitiven `match_scores`-Spalten nachgezogen (entdeckt beim DSGVO-Datenexport, TEN-66). Diese Spalten dienen Mandanten-Trennung und Nachvollziehbarkeit und sind im ER-Diagramm bewusst nicht einzeln dargestellt.
